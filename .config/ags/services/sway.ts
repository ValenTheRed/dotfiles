...
export class Sway extends Service {
    ...
    constructor() {
        ...
        ["monitor", "workspace", "client"].forEach((active) =>
            this._active.connect(`notify::${active}`, () =>
                this.notify("active"),
            ),
        );
    }
    ...
    private _watchSocket(stream: Gio.InputStream) {
        stream.read_bytes_async(
            ...
            (_, resultHeader) => {
                ...
                if (!data) return;
                const payloadLength = new Uint32Array(
                    data.slice(6, 10).buffer,
                )[0];
                const payloadType = new Uint32Array(
                    data.slice(10, 14).buffer,
                )[0];
                stream.read_bytes_async(
                    payloadLength,
                    GLib.PRIORITY_DEFAULT,
                    ...
                    (_, resultPayload) => {
                        const data = stream
                            .read_bytes_finish(resultPayload)
                            .get_data();
                        if (!data) return;
                        this._onEvent(
                            payloadType,
    ...
    private _handleWindowEvent(clientEvent: Client_Event) {
        ...
        switch (clientEvent.change) {
            ...
            case "focus":
                ...
                const current_active = this._clients.get(
                    this._active.client.id,
                );
