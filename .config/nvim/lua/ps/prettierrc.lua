...
local function config(bufnr)
  ...
  local opts = {}
  for _, rc in ipairs(vim.fs.find(".prettierrc.js", {
    path = path, upward = true, stop = vim.fn.expand("~"), type = "file"
  })) do
    opts = parse(rc)
  end
  ...
  for opt, val in pairs(opts) do
    ...
    if func then
      ...
      if ok then
        ...
        warn('prettierrc->editorconfig: invalid value for option %s: %s. %s', opt, val, err)
      end
    end
  end
